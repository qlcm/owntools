args <- commandArgs(T)
USAGE <- function(){
         message("USAGE:\n\tRscript bin.Cons.plot.R <regionfile(chrom,start,end)> <valuefilepath> <promoter_regionfile> <enhancer_regionfile> <binnum> <regionsize> <outpath(filepath)> <tissname>\n")
}
USAGE()
...
value.split <- fread(paste(args[2],"/",t,sep=""))
colnames(value.split) <- c("chrom","start","end","score")
value.split <- value.split[value.split$score >0,]
value.split$start <- as.integer(value.split$start + 1)
value.split$length <- as.integer(value.split$end - value.split$start)
...
random.result <- random.addvalue[,.(random = mean(score)),by=bin.id]
